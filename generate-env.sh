...
# Ports du TUNNEL local (Option A)
MYSQL_TUNNEL_HOST="host.docker.internal"
MYSQL_TUNNEL_PORT="3307"     # local -> RDS:3306 via bastion
...
# Endpoint RDS (réel, dans le VPC) – utile pour info/logs
CATALOG_DB_ENDPOINT=${RDS_ENDPOINT}:3306

# Endpoint que l'app DOIT utiliser (tunnel SSH local)
MYSQL_HOST=${MYSQL_TUNNEL_HOST}
...
# Pour les apps qui préfèrent une URL/DSN unique (facultatif)
...
echo "N’oublie pas d’ajouter .env à ton .gitignore."
